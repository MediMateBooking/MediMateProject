...
router.get("/checkpoint/api", async (req, res) => {
    ...
    try{
        ...
        const [patientAccount, doctorAccount] = await Promise.all([
          db.DbConn().collection("patients").findOne({ accountValidationToken: token, linkExpire: { $gt: Date.now() } }),
          db.DbConn().collection("doctors").findOne({ emailValidationToken: token, linkExpire: { $gt: Date.now() } })
        ]);
          ...
          if(patientAccount){
            ...
            const updateActiveStatus = {
              ...
              const UpdatedUserActiveStatus = await db
                .DbConn()
                .collection("patients")
                .updateOne({ accountValidationToken: token }, { $set: updateActiveStatus });
          ...
          if(doctorAccount){
            ...
            .DbConn()
            .collection("doctors")
            .updateOne({ emailValidationToken: token }, { $set: updateEmailValidationStatus });
